...
def join(model_id2m_ids_groups, model_id2S):
    ...
    N = block_diag(*[S.N[sorted(S.m_id2i[m_id] for m_id in S.m_ids if (model_id, m_id) not in common_m_ids), :]
                     ...
                     for (model_id, S) in model_id2S.items()])
    ...
    m_id2new_m_id = dict()
    for model_id2m_ids in model_id2m_ids_groups:
        model_id2m_row = {model_id: zeros(shape=(1, len(S.r_ids))) for model_id, S in model_id2S.items()}
        new_m_id = tuple(((model_id, tuple(m_ids)) for (model_id, m_ids) in model_id2m_ids.items()))
        m_id2i[new_m_id] = m_shift
        if next((True for (model_id, m_ids) in model_id2m_ids.items()
        ...
        row = hstack(tuple(model_id2m_row[model_id] for model_id in model_id2S.keys()))
        N = vstack((N, row))
        m_shift += 1
