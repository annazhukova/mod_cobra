...
def analyse_model_efm(sbml, out_r_id, out_rev, res_dir, in_m_id, out_m_id, in_r_id2rev, tree_efm_path,
    ...
    system = get_initial_system(sbml, model, res_dir, out_r_id, out_rev, in_r_id2rev, in_m_id, out_m_id,
                                tree_efm_path, max_efm_number, threshold)
    logging.info('Computed initial system')
    if not system.efm_id2i:
        return system, defaultdict(lambda: 1)

    system.lump_coupled_reactions()
    system.remove_unused_metabolites()

    r_id2w = defaultdict(lambda: 1)
    r_id2w.update({gr_r_id: len(r_id2c) for (gr_r_id, r_id2c) in system.gr_id2r_id2c.iteritems()})
