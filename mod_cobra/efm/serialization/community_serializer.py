    ...
    with open(comm_txt, 'w+') as f:
        ...
        for clu_id in sorted(id2cluster.iterkeys()):
            ...
            for fm_id in cluster:
                ...
                r_id2count.update({it: 1 for it in r2st.iterkeys()})
                p_id2count.update({it: 1 for it in p2st.iterkeys()})
            ...
            def format_class2ms(class2m_ids, m_id2count):
                ...
                result = '\n\t'.join('%sS: %s' % (name.upper(), format_m_ids(m_ids)) for (name, m_ids) in class2m_ids.iteritems())
                others = set(m_id2count.iterkeys()) - reduce(lambda s1, s2: s1 | s2, class2m_ids.itervalues(), set())
                if others:
            ...
            #             % ', '.join(('%s [%d]' % (pw, count)
            #                          for (pw, count) in sorted(pws.iteritems(), key=lambda (pw, c): (-c, pw)))))
    return comm_txt
