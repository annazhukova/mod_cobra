...
from mod_cobra.fbva.fbva_analyser import analyse_by_fva
...
class FVATestCase(unittest.TestCase):
    ...
    def test_fva_ess_rxns_num(self):
        cobra_model = create_cobra_model_from_sbml_file(TEST_SBML)
        r_id2bounds, _ = analyse_by_fva(cobra_model, bm_r_id='r3', objective_sense='maximize')
        essential_rs_num = len([1 for (l_b, u_b) in r_id2bounds.values() if l_b * u_b > 0])
    ...
    def test_fva_ess_rxn_in(self):
        cobra_model = create_cobra_model_from_sbml_file(TEST_SBML)
        r_id2bounds, _ = analyse_by_fva(cobra_model, bm_r_id='r3', objective_sense='maximize')
        essential_rs = {r_id for (r_id, (l_b, u_b)) in r_id2bounds.items() if l_b * u_b > 0}
    ...
    def test_fva_var_rxn_num(self):
        cobra_model = create_cobra_model_from_sbml_file(TEST_SBML)
        r_id2bounds, _ = analyse_by_fva(cobra_model, bm_r_id='r3', objective_sense='maximize')
        variable_rs = {r_id for (r_id, (l_b, u_b)) in r_id2bounds.items() if l_b * u_b <= 0}
